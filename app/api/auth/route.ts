 ...
 * object { id, email, password, role } and `verifyPassword` for bcrypt comparison.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!staff) {
      if (email === "admin@example.com" && password === "password") {
        ...
        const res = NextResponse.json(
          { user: { id: payload.sub, email, role: "admin" } },
          { status: 200 },
        );
    ...
    if (db && typeof db.verifyPassword === "function") {
      const isPasswordValid = await db.verifyPassword(password, staff.password);
      if (!isPasswordValid) {
        return NextResponse.json(
          { message: "Invalid credentials" },
          { status: 401 },
      ...
      // Fallback to plaintext comparison if verifyPassword isn't available
